...
input char num=1;             //The most misplaced bbandstops to adjust
input bool barprice=1;        //Highest/Lowest value if true otherwise open/close value
...
char barleavemavote()
  {
   if(barprice)
     {
      if(High[1]<iMA(NULL,0,period,0,method,0,0))
        {
         return SELL;
        }
      else if(Low[1]>iMA(NULL,0,period,0,method,0,0))
        {
         return BUY;
        }
     }
     ...
     {
      if(Open[1]<iMA(NULL,0,period,0,method,0,0) && Close[1]<iMA(NULL,0,period,0,method,0,0))
        {
         return SELL;
        }
      else if(Open[1]>iMA(NULL,0,period,0,method,0,0) && Close[1]>iMA(NULL,0,period,0,method,0,0))
        {
         return BUY;
        }
     }
...
char bbvote()
  {
   ...
   bb1=iCustom(NULL,0,"::Indicators\\Downloads\\BBand Stop Alert.ex4",length1,2,1.0,1,1,1000,false,0,(onetick?1:0));/*UpTrend Stop*/
   bb2=iCustom(NULL,0,"::Indicators\\Downloads\\BBand Stop Alert.ex4",length2,2,1.0,1,1,1000,false,0,(onetick?1:0));/*UpTrend Stop*/
   if(bb1>0 && misplace?(bb2>=bb1):(bb2>0))
     {
      for(char i=1+onetick;i<=num+onetick;i++)
        {
         bb1=iCustom(NULL,0,"::Indicators\\Downloads\\BBand Stop Alert.ex4",length1,2,1.0,1,1,1000,false,0,i);/*UpTrend Stop*/
         bb2=iCustom(NULL,0,"::Indicators\\Downloads\\BBand Stop Alert.ex4",length2,2,1.0,1,1,1000,false,0,i);/*UpTrend Stop*/
         if(i==num+onetick)
           {
            if(bb1<0||bb2<0);else return NONE;
           }
         else
           {
            if(bb1>0 && misplace?(bb2>=bb1):(bb2>0));else return NONE;
           }
        }
      return BUY;
     }
   bb1=iCustom(NULL,0,"::Indicators\\Downloads\\BBand Stop Alert.ex4",length1,2,1.0,1,1,1000,false,1,(onetick?1:0));
   bb2=iCustom(NULL,0,"::Indicators\\Downloads\\BBand Stop Alert.ex4",length2,2,1.0,1,1,1000,false,1,(onetick?1:0));
   if(bb2>0 && misplace?(bb2<=bb1):(bb1>0))
     {
      for(char i=1+onetick;i<=num+onetick;i++)
        {
         bb1=iCustom(NULL,0,"::Indicators\\Downloads\\BBand Stop Alert.ex4",length1,2,1.0,1,1,1000,false,1,i);/*UpTrend Stop*/
         bb2=iCustom(NULL,0,"::Indicators\\Downloads\\BBand Stop Alert.ex4",length2,2,1.0,1,1,1000,false,1,i);/*UpTrend Stop*/
         if(i==num+onetick)
           {
            if(bb1<0||bb2<0);else return NONE;
           }
           ...
           {
            if(bb2>0 && misplace?(bb2<=bb1):(bb1>0));else return NONE;
           }
        }
      return SELL;
     }
